...
| 模块 | 文件 | 状态 |
|------|------|:----:|
| 事件定义 | `handlers/diagram_event.dart` | ✅ |
| 上下文定义 | `handlers/diagram_context.dart` | ✅ |
| 处理器基类 | `handlers/diagram_handler.dart` | ✅ |
| 处理器注册表 | `handlers/handler_registry.dart` | ✅ |
| 锚点处理器 | `handlers/anchor_click_handler.dart` | ✅ |
| 节点拖动处理器 | `handlers/node_drag_handler.dart` | ✅ |
| 框选处理器 | `handlers/selection_handler.dart` | ✅ |
| 画布平移处理器 | `handlers/canvas_pan_handler.dart` | ✅ |
| 空间索引 | `spatial/simple_index.dart` | ✅ |
| 命令基类 | `commands/diagram_command.dart` | ✅ |
| 历史控制器 | `commands/history_controller.dart` | ✅ |
| ER 交互管理器 | `integration/er_interaction_manager.dart` | ✅ |

### V2 核心问题：InteractiveViewer

**问题描述**：

V2 画布使用 `InteractiveViewer` 包裹节点，但 InteractiveViewer 会拦截所有手势事件：
- 即使设置 `panEnabled=false`，InteractiveViewer 内部的 `GestureDetector` 仍然存在
- 当用户点击节点时，事件可能被 InteractiveViewer 捕获而非传递到节点 Widget

**V1 解决方案**：

V1 通过双重策略解决这个问题：
1. **画布 Listener (translucent)** - 先接收事件，做手动命中测试
2. **节点 Listener (opaque)** - 拦截事件，阻止传递给 InteractiveViewer

```
画布 Listener (translucent) ← 先接收，做命中测试
    ↓
InteractiveViewer (panEnabled=false) ← 可能拦截
    ↓
节点 Listener (opaque) ← 如果能到达，则拦截阻止继续传递
    ↓
...
V2 假设只要 HandlerRegistry 正确分发事件，就能替代 V1 的复杂逻辑。但忽略了：
1. HandlerRegistry 需要在事件到达之后才能工作
2. InteractiveViewer 阻断了事件传递链
3. 节点 Widget 根本收不到事件

### V2 修复方案建议

#### 方案 A：保留 InteractiveViewer，增加手动命中测试
...
Listener(
  behavior: HitTestBehavior.translucent,
  onPointerDown: (event) {
    // 1. 手动命中测试（在 InteractiveViewer 之前）
    final scenePos = toScene(event.localPosition);
    final hitResult = spatialIndex.hitTest(scenePos);

    if (hitResult.isOnNode || hitResult.isOnAnchor) {
      // 2. 直接调用 HandlerRegistry 分发
      final context = buildContext(event, hitResult);
      handlerRegistry.dispatch(event, context);
    } else {
      // 3. 让 InteractiveViewer 处理
    }
  },
...
#### 方案 B：移除 InteractiveViewer，完全手动处理

```dart
// 完全移除 InteractiveViewer，使用 Listener + 手动变换
Listener(
  behavior: HitTestBehavior.opaque,
  onPointerDown: (event) => handlerRegistry.dispatch(event, context),
  onPointerMove: (event) => handlerRegistry.dispatch(event, context),
  onPointerUp: (event) => handlerRegistry.dispatch(event, context),
  onPointerSignal: (event) => handleScroll(event), // 滚轮缩放
  child: Transform(
...
Listener(
  ...
  onPointerDown: (event) {
    // 编辑模式：手动命中测试 + 直接分发
    if (isEditMode) {
      final hitResult = spatialIndex.hitTest(toScene(event.localPosition));
      if (hitResult.isOnElement) {
        // 手动处理，不传递给 InteractiveViewer
        handlerRegistry.dispatch(event, buildContext(event, hitResult));
        return;
      }
    }
    // 其他情况：让 InteractiveViewer 处理
  },
  child: InteractiveViewer(
    panEnabled: isPreviewMode,
    child: Stack(
      children: [
        // 节点层 - 仍然需要 Listener 拦截
        for (final node in nodes)
          Positioned(
            left: node.x,
            top: node.y,
            child: Listener(
              behavior: HitTestBehavior.opaque, // 拦截事件
              onPointerDown: (_) {}, // 阻止传递给 InteractiveViewer
              child: NodeWidget(...),
            ),
          ),
      ],
...
推荐采用**方案 C（混合方案）**：
1. 保留 InteractiveViewer 用于预览模式的平移/缩放
2. 编辑模式使用手动命中测试 + HandlerRegistry 分发
3. 节点保留 Listener(opaque) 拦截器确保事件传递

这样既能利用 V2 的 HandlerRegistry 架构，又能保证事件正确传递。
