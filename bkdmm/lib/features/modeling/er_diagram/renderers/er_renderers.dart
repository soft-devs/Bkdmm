...
class ERNodeRenderer extends BaseNodeRenderer {
  ...
  void paint({
    ...
    // 2. 绘制背景
    final bgColor = TDAppTheme.getNodeBgColor(context.isDarkMode, state.isSelected);
    ...
    if (state.isHighlighted) {
      drawHighlightBorder(canvas, rect, color: TDAppTheme.getHighlightBorderColor());
    }
  ...
  void _drawHeader(Canvas canvas, Rect rect, Entity entity, bool isDarkMode) {
    ...
    // 表头背景
    final headerColor = TDAppTheme.getNodeHeaderColor(isDarkMode);
  ...
  void _drawFieldAnchors(Canvas canvas, Rect rect, Entity entity, bool isDarkMode) {
    for (var i = 0; i < entity.fields.length; i++) {
      ...
      AnchorRenderer.paintFieldAnchor(
        canvas,
        leftAnchor,
        color: TDAppTheme.getAnchorColor(field.pk),
...
class EREdgeRenderer extends BaseEdgeRenderer {
  ...
  void paint({
    ...
    final color = state.isHighlighted
        ? TDAppTheme.getHighlightBorderColor()
        : TDAppTheme.getEdgeColor(context.isDarkMode);
